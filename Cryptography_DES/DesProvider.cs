...
namespace Cryptography.TripleDes
{
    public class DesProvider
    {
        ...
        public BitArray RoundFunction(BitArray input, BitArray key)
        {
            var expandedBox = Expand(input);
            var xored = expandedBox.Xor(key);
            var boxed = SBoxSubstitution(xored);
            var permutated = FinalSboxPermutation(boxed);
            return permutated;
        }

        private BitArray Expand(BitArray input)
        {
            var array = new bool[48];

            for (var i = 0; i < 48; i++)
                array[i] = input[_expansionVector[i] - 1];
        ...
        private BitArray SBoxSubstitution(BitArray input)
        {
            ...
            for (var i = 0; i < 8; i++) //8 boxes
            {
                boxes[i] = new BitArray(6);
                for (var j = 0; j < 6; j++) //6 bits each
        ...
        public BitArray InitialPermutation(BitArray bitArray)
        {
            var array = new bool[bitArray.Length];

            for (var i = 0; i < 64; i++)
                array[i] = bitArray[_initialPermutationVector[i] - 1];
        ...
        public BitArray FinalPermutation(BitArray bitArray)
        {
            ...
            for (var i = 0; i < 64; i++)
                array[i] = bitArray[_finalPermutationVector[i] - 1];

            return new BitArray(array);
        }

        private BitArray FinalSboxPermutation(BitArray bitArray)
        {
            var array = new bool[bitArray.Length];
        ...
        #region PermutationVectors

        private readonly int[] _initialPermutationVector =
        {
        ...
        private readonly int[] _finalPermutationVector = new int[64]
        {
        ...
        private readonly int[] _expansionVector =
        {
